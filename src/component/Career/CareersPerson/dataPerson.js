import Person1 from "../../../assets/img/person1.jpeg";
import person2 from "../../../assets/img/person2.jpeg";
import person3 from "../../../assets/img/person3.jpeg";
import person4 from "../../../assets/img/person4.jpeg";
import person5 from "../../../assets/img/person5.jpeg";
import person6 from "../../../assets/img/person6.jpeg";

export const people = [
  {
    img: Person1,
    alt: "person1",
    name: "Saleem Fadel",
    careerName: "Company Manager",
    description:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nunc aliquam justo et nibh venenatis aliquet. Morbi mollis mollis pellentesque. Aenean vitae erat velit.",
  },
  {
    img: person2,
    alt: "person2",
    name: "Neveen Nasaeer",
    careerName: "Leidos",
    description:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nunc aliquam justo et nibh venenatis aliquet. Morbi mollis mollis pellentesque. Aenean vitae erat velit. Maecenas urna sapien, dignissim a augue vitae, porttitor luctus urna. Morbi scelerisque semper congue. Donec vitae congue quam.",
  },
  {
    img: person3,
    alt: "person3",
    name: "Salman Fadel",
    careerName: "Idex Consulting",
    description:
      "Aenean vitae fringilla metus, sed pellentesque nisl. Morbi quis mi tincidunt, consequat risus et, dapibus magna. In lacinia dictum nunc, non dignissim quam.",
  },
  {
    img: person4,
    alt: "person4",
    name: "Noha Fadel",
    careerName: "Idex Consulting",
    description:
      "Donec vitae congue quam. Pellentesque convallis est a eros porta, ut porttitor magna convallis. Donec quis felis imperdiet, vestibulum est ut, pulvinar dolor.",
  },
  {
    img: person5,
    alt: "person5",
    name: "Reham Fadel",
    careerName: "Idex Consulting",
    description:
      "Aenean vitae fringilla metus, sed pellentesque nisl. Morbi quis mi tincidunt, consequat risus et, dapibus magna. In lacinia dictum nunc, non dignissim quam. Aliquam lorem lectus, vulputate non lobortis sed, auctor in ante.",
  },
  {
    img: person6,
    alt: "person6",
    name: "Mona Fadel",
    careerName: "Idex Consulting",
    description:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Praesent ultricies ut nibh nec cursus. Curabitur porttitor nibh a tellus egestas, sit amet interdum nisl imperdiet. Fusce imperdiet diam odio, ut viverra metus fringilla vel. Nulla aliquam porttitor libero. Cras...",
  },
];
